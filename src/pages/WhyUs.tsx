            ...
            {whyUs.map((item, i) => {
              return (
                <motion.div
                  ...
                  className="group rounded-2xl border border-slate-200 bg-slate-100 p-6 shadow-md backdrop-blur-2xl transition duration-300 hover:-translate-y-1 hover:shadow-lg md:p-8 card-gradient-border-hover"
                  initial={{ opacity: 0, y: 24 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  viewport={{ once: true, amount: 0.3 }}
                  transition={{ duration: 0.55, delay: i * 0.08 }}
                >
                  <div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-slate-200 bg-slate-200/50 text-2xl transition group-hover:border-brand-blue/30 group-hover:bg-slate-200">
                    {item.icon}
                  </div>
